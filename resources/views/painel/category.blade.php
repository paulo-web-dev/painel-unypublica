...
@section('content')
    ...
    <a href="{{ route('adicionar-categoria') }}" class="btn btn-primary w-49 mr-2 mt-5 mb-2"> <i data-feather="plus"
            class="w-4 h-4 mr-2"></i> Adicionar Categoria!
    </a>
                ...
                <tbody>
                    @foreach ($categories as $category)
                        <tr class="intro-x">
                            ...
                            <td class="text-center">
                                <a href="{{ route('informacao-curso', ['course' => $category->id]) }}"
                                    class="font-medium whitespace-nowrap">{{ $category->title }}</a>
                                <div class="text-gray-600 text-xs whitespace-nowrap mt-0.5">{{ $category->subtitle }}</div>
                            </td>
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center text-theme-1 mr-3"
                                        href="{{ route('informacao-categoria', ['category' => $category->id]) }}">
                                        <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Editar
                                    </a>
                                    <a class="flex items-center text-theme-6 mr-3" href="javascript:;" data-toggle="modal"
                                        data-target="#excluirCurso{{ $category->id }}">
                                        <i data-feather="trash" class="w-4 h-4 mr-1"></i> Excluir
                                    </a>
                                </div>
                                <!-- BEGIN: Modal Content -->
                                <div id="excluirCurso{{ $category->id }}" class="modal" tabindex="-1"
                                    aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form action="{{ route('excluir-categoria', ['category' => $category->id]) }}"
                                                method="POST">
